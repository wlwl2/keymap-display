...
function resetFilter () {
  var searchInput = document.querySelector('.atom-keymap-display-package atom-text-editor')
  var allKeys = document.querySelectorAll('.all-keys')
  for (var j = 0; j < allKeys.length; j++) {
    allKeys[j].setAttribute('data-show-key', 'no')
  }
  searchInput.getModel().setText('')
}
...
export default {
  ...
  toggle () {
    ...
    var searchInput = document.querySelector('.atom-keymap-display-package atom-text-editor')
    var allKeys = document.querySelectorAll('.all-keys')
    searchInput.getModel().element.focus()
    searchInput.getModel().onDidChange(function (result) {
      selectAllGroup()
      ...
      for (var i = 0; i < allKeys.length; i++) {
        if (allKeys[i].firstChild.firstChild.textContent.indexOf(searchInput.getModel().getText()) !== -1) {
